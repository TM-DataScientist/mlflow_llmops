...
【このファイルの使われ方】
  log_model.py の mlflow.langchain.log_model(lc_model=MODEL_CODE_PATH, ...) から
  このファイルのパスが渡される。MLflow はこのファイルをアーティファクトとして保存し、
  mlflow.langchain.load_model() 呼び出し時にコードを再実行してモデルを復元する。
